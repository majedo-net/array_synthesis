function [xs,ys,time_delays]=CalcArrayFactor(freq,d,theta_0,phi_0,options)
%%
...
rs = sqrt(xs.^2 + ys.^2);
phase_shifts = (xs.*sin(theta_0).*cos(phi_0)+ys.*sin(theta_0).*sin(phi_0));
time_delays = (phase_shifts-min(phase_shifts))./(2*pi) * (1/freq);
steering_vectors = exp(-1j.*k.*phase_shifts);
...
for n = 1:size(xs)
    ...
    ang_pos3 = reshape(ang_pos(n),1,1,[]);
